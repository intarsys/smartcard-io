...
public class Connect {
	...
	public void run() throws Exception {
		...
		List<PCSCStatusMonitor> activeMonitors = new ArrayList<PCSCStatusMonitor>();
		for (IPCSCCardReader reader : readers) {
			...
			monitor.addStatusListener(new IStatusListener() {
				...
				public void onStatusChange(IPCSCCardReader reader, PCSCCardReaderState cardReaderState) {
					System.out.println("reader " + cardReaderState.getReader() + " state " + cardReaderState);
		...
		while (!activeMonitors.isEmpty() && System.in.available() == 0) {
			Thread.sleep(100);
		}
		context.dispose();
	}
